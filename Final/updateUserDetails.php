...
$sql = "select name, surname, siteName, country, industry, receiveEmails, siteCharacter from userDetails where id = ?";
...
if($stmt = mysqli_prepare($link, $sql)){
    ...
    if(mysqli_stmt_execute($stmt)){
        ...
        $_SESSION["siteName"] = $row[2];
        $_SESSION["country"] = $row[3];
        $_SESSION["industry"] = $row[4];
        $_SESSION["receiveEmails"] = $row[5];
        $_SESSION["siteCharacter"] = $row[6];
...
$firstNameInitial = $lastNameInitial = $countryInitial = $industryInitial = $receiveEmailsInitial = $siteCharacterInitial = $siteNameInitial = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($firstName_err) && empty($lastName_err) && empty($siteName_err) && empty($industry_err) && empty($country_err)){
        ...
        $sql = "update userDetails set name = ?, surname = ?, siteName = ?, country = ?, industry = ? where id = ?";
        
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssi", $param_firstName, $param_lastName, $param_siteName, $param_country, $param_industry, $param_id);
            ...
            if(mysqli_stmt_execute($stmt)){                    
                    ...
                    for($i = 0; $i < $examsLength; $i++){
                        $sql = "update userExams set passedIndicator = ?, registeredIndicator = ? where id = ? and code = ?";
                        $stmt = mysqli_prepare($link, $sql);
                        mysqli_stmt_bind_param($stmt,"iiis",$param_ind, $param_regInd,$param_id,$param_code);
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group <?php echo (!empty($lastName_err)) ? 'has-error' : ''; ?>">
                ...
                <input type="text" name="lastName" class="form-control" value="<?php echo $_SESSION["surname"]; ?>">
            ...
            <div class="form-group <?php echo (!empty($siteName_err)) ? 'has-error' : ''; ?>">
                ...
                <input type="text" name="siteName" class="form-control" value="<?php echo $_SESSION["siteName"]; ?>">
